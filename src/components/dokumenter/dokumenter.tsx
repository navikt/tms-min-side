...
const Dokumenter = ({ language }: Props) => {
  const {
    ...
    isLoading,
    error,
  } = useSWRImmutable<DokumentType[]>({ path: dokumenterUrl, options: include }, fetcher);
  ...
  return (
    <>
      ...
      <ul className={styles.dokumenter}>
        {dokumenter?.slice(0, 3).map((dokument) => <Dokument key={dokument.dokumentInfoId} dokument={dokument} />)}
        <div className={styles.alle}>
          <a
            className={styles.link}
            href={`${dokumentarkivUrl}/${language}`}
            onClick={() => logEvent("dokumentwidget", "dokumenter", "Se alle dokumenter")}
          >
